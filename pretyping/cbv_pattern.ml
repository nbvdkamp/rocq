...
(** analogous to types for constrs, intended to represent patterns that are a type **)
type types_pattern = constr_pattern

(** analogous to definitions of (co)fixpoint for constrs (in constr.ml) **)
...
type fixpoint_pattern = (constr_pattern, types_pattern, Sorts.relevance) pfixpoint
type ('constr, 'types, 'r) pcofixpoint =
    int * ('constr, 'types, 'r) prec_declaration
type cofixpoint_pattern = (constr_pattern, types_pattern, Sorts.relevance) pcofixpoint *)

type fixpoint_pattern = (int array * int) * (Name.t array * types_pattern array * constr_pattern array)
type cofixpoint_pattern = int * (Name.t array * types_pattern array * constr_pattern array)
 ...
 *)
type cbv_value =
  | VAL of int * constr_pattern
  | STACK of int * cbv_value * cbv_stack
  | LAMBDA of int * (Name.t * types_pattern) list * constr_pattern * cbv_value subs
  | PROD of Name.t  * types_pattern * types_pattern * cbv_value subs
  | LETIN of Name.t * cbv_value * types_pattern option * constr_pattern * cbv_value subs (* CHANGE: made type option *)
  | FIX of fixpoint_pattern * cbv_value subs * cbv_value list
  | COFIX of cofixpoint_pattern * cbv_value subs * cbv_value list
  | CONSTRUCT of constructor * cbv_value list
  (* | PRIMITIVE of CPrimitives.t * Constant.t * cbv_value list *)
  | ARRAY of cbv_value Parray.t * cbv_value
  | SYMBOL of { cst: Constant.t; unfoldfix: bool; rules: Declarations.machine_rewrite_rule list; stk: cbv_stack }
...
let rec shift_value n = function
  ...
  | ARRAY (t,ty) ->
      ARRAY(Parray.map (shift_value n) t, shift_value n ty)
...
module KeyTable = Hashtbl.Make(struct
  ...
  let equal = Names.eq_table_key eq_constant_key
  let hash = Names.hash_table_key (fun c -> Constant.UserOrd.hash c)
end)
...
let debug_cbv = CDebug.create ~name:"Cbv_pattern" ()
...
module VNativeEntries =
  struct
    ...
    let mkInt env i = VAL(0, PInt i)

    let mkFloat env f = VAL(0, PFloat f)

    let mkString env s = VAL(0, PString s)

    let mkBool env b =
      let (ct,cf) = get_bool_constructors env in
      CONSTRUCT((if b then ct else cf), [])

    let int_ty env = VAL(0, PRef(ConstRef(get_int_type env)))

    let float_ty env = VAL(0, PRef(ConstRef(get_float_type env)))

    let mkCarry env b e =
      let (c0,c1) = get_carry_constructors env in
      CONSTRUCT((if b then c1 else c0), [int_ty env;e])

    let mkIntPair env e1 e2 =
      let int_ty = int_ty env in
      let c = get_pair_constructor env in
      CONSTRUCT(c, [int_ty;int_ty;e1;e2])

    let mkFloatIntPair env f i =
      let float_ty = float_ty env in
      let int_ty = int_ty env in
      let c = get_pair_constructor env in
      CONSTRUCT(c, [float_ty;int_ty;f;i])
...
let debug_pr_key = function
  | ConstKey sp -> Names.Constant.print sp
...
and reify_value = function (* reduction under binders *)
  | VAL (n,t) -> lift_pattern n t
  | STACK (0,v,stk) ->
      reify_stack (reify_value v) stk
  | STACK (n,v,stk) ->
      lift_pattern n (reify_stack (reify_value v) stk)
  | PROD(na,t,u,env) ->
    apply_env env (PProd (na,t,u))
  | LETIN(na,b,t,c,env) ->
    apply_env env (PLetIn (na,reify_value b,t,c))
  | LAMBDA (k,ctxt,b,env) ->
    apply_env env @@
    List.fold_left (fun c (n,t) ->
        PLambda (n, t, c)) b ctxt
  | FIX ((lij,fix),env,args) ->
    let fix = PFix (lij, fix) in
    PApp (apply_env env fix, Array.map_of_list reify_value args)
  | COFIX ((j,cofix),env,args) ->
    let cofix = PCoFix (j, cofix) in
    PApp (apply_env env cofix, Array.map_of_list reify_value args)
  | CONSTRUCT (c,args) ->
      PApp(PRef(ConstructRef c), Array.map_of_list reify_value args)
  (* | PRIMITIVE(op,c,args) ->
      PApp(PRef(ConstRef c), Array.map_of_list reify_value args) *)
  | ARRAY (t,ty) ->
    let t, def = Parray.to_array t in
      PArray(Array.map reify_value t, reify_value def, reify_value ty)
  | SYMBOL { cst; stk; _ } ->
      reify_stack (PRef(ConstRef cst)) stk
...
and apply_env env t =
  ...
  | _ ->
    map_pattern_with_binders (fun _ -> subs_lift) apply_env env t
...
let expand_branch env pms (ind, i) br =
...
let rec norm_head info env pattern stack =
  (* no reduction under binders *)
  match pattern with
  (* stack grows (remove casts) *)
  | PApp (head,args) -> (* Applied terms are normalized immediately;
  ...
  | PRel i ->
      (match expand_rel i env with
        ...
        | Inr (n,None)   -> (VAL(0, PRel n), stack)
        | Inr (n,Some p) -> norm_head_ref (n-p) info env stack (RelKey p) pattern)

  | PVar id -> norm_head_ref 0 info env stack (VarKey id) pattern
  ...
  (* neutral cases *)
  | (PSort _ | PMeta _ | PRef IndRef _ | PInt _ | PFloat _ | PString _) -> (VAL(0, pattern), stack)
  | PProd (na,t,u) -> (PROD(na,t,u,env), stack)
...
and norm_head_ref k info env stack normt t =
  if red_set_ref info.reds normt then
    match cbv_value_cache info normt with
      ...
      | Declarations.Primitive op -> raise PatternFailure
...
and cbv_stack_value info env = function
  ...
  | (CONSTRUCT(((sp,n),_),[]), APP(args,CASE(pms,_p,br,iv,ci,env,stk)))
    ...
    let env =
      if (Int.equal ci.ci_cstr_ndecls.(n - 1) ci.ci_cstr_nargs.(n - 1)) then (* no lets *)
        ...
        let ctx = expand_branch info.env pms (sp, n) br in
  ...
  | SYMBOL ({ cst; rules; stk } as s ), stk' ->
    ...
    begin try
      let rhs, stack = cbv_apply_rules info env cst rules stk in
        ...
        let body = match ref with
          | RelKey n ->
            ...
            begin match Environ.lookup_rel n info.env with
              | LocalDef (_, c, _) -> lift_pattern n c
...
and it_mkLambda_or_LetIn info ctx t =
  ...
  | LocalAssum (n, ty) :: ctx ->
      ...
      LAMBDA (List.length assums, assums, it_mkPLambda_or_LetIn (reify_value t) (List.rev ctx), subs_id 0)
  | LocalDef _ :: _ ->
      cbv_stack_term info TOP (subs_id 0) (it_mkPLambda_or_LetIn (reify_value t) ctx)
...
and cbv_match_rigid_arg_pattern info env ctx psubst p t =
  ...
  | PHInd (ind, pu), VAL(0, t') ->
    begin match t' with PRef IndRef ind' when Environ.QInd.equal info.env ind ind' -> match_instance pu psubst | _ -> raise PatternFailure end
  | PHConstr (constr, pu), CONSTRUCT (constr', []) ->
    if Environ.QConstruct.equal info.env constr constr' then match_instance pu psubst else raise PatternFailure
  | PHRel i, VAL(k, t') ->
    begin match t' with PRel n when Int.equal i (k + n) -> psubst | _ -> raise PatternFailure end
  | PHSort ps, VAL(0, t') ->
    begin match t' with PSort s -> match_sort ps s psubst | _ -> raise PatternFailure end
  | PHSymbol (c, pu), SYMBOL { cst = c'; _ } ->
    if Environ.QConstant.equal info.env c c' then match_instance pu psubst else raise PatternFailure
  | PHInt i, VAL(0, t') ->
    begin match t' with PInt i' when Uint63.equal i i' -> psubst | _ -> raise PatternFailure end
  | PHFloat f, VAL(0, t') ->
    begin match t' with PFloat f' when Float64.equal f f' -> psubst | _ -> raise PatternFailure end
  | PHString s, VAL(0, t') ->
    begin match t' with PString s' when Pstring.equal s s' -> psubst | _ -> raise PatternFailure end
...
and cbv_apply_rule info env ctx psubst es stk =
  ...
  | Declarations.PECase (pind, pret, pbrs) :: e, CASE (pms, (p, _), brs, iv, ci, env, s) ->
      ...
      let ntys_brs = Inductive.expand_branch_contexts specif UVars.Instance.empty pms brs in
...
and cbv_apply_rules info env u r stk =
  ...
  | { lhs_pat = (pu, elims); nvars; rhs } :: rs ->
    try
      ...
      let psubst = match_instance pu psubst in
...
and cbv_norm_value info = function
  ...
  | STACK (n,v,stk) ->
      lift_pattern n (apply_stack info (cbv_norm_value info v) stk)
  | PROD(na,t,u,env) ->
      PProd (na,cbv_norm_term info env t,cbv_norm_term info (subs_lift env) u)
  | LETIN (na,b,t,c,env) ->
      let aux = if info.strong then cbv_norm_term info else apply_env in
      PLetIn (na,cbv_norm_value info b, Option.map (aux env) t,aux (subs_lift env) c)
  | LAMBDA (n,ctxt,b,env) ->
      let nctxt =
  ...
  | COFIX ((j,(names,lty,bds)),env,args) ->
      PApp
        (PCoFix (j,
         ...
         Array.map_of_list (cbv_norm_value info) args)
  | CONSTRUCT (c,args) ->
      PApp(PRef(ConstructRef c), Array.map_of_list (cbv_norm_value info) args)
  (* | PRIMITIVE(op,c,args) ->
      PApp(PRef(ConstRef c),Array.map_of_list (cbv_norm_value info) args) *)
  | ARRAY (t,ty) ->
    let ty = cbv_norm_value info ty in
    let t, def = Parray.to_array t in
    let def = cbv_norm_value info def in
      PArray(Array.map (cbv_norm_value info) t, def, ty)
  | SYMBOL { cst; stk; _ } -> apply_stack info (PRef(ConstRef cst)) stk

(* with profiling *)
let cbv_norm_pattern infos pattern =
  cbv_norm_term infos (subs_id 0) pattern

(* constant bodies are normalized at the first expansion *)
let create_cbv_pattern_infos reds ~strong env sigma =
