...
let warn_meta_collision =
  CWarnings.create ~name:"meta-collision-unfolding" ~category:CWarnings.CoreCategories.ltac
         (fun name ->
...
let constrain env sigma n (ids, m) ((names,seen as names_seen), terms as subst) =
  ...
  try
    ...
    if List.equal Id.equal ids ids' && eq_constr_mod_unfolding env sigma m m' then subst
...
let matches_core env sigma allow_bound_rels (binding_vars, pat) c =
  ...
  | PSoApp (n, args), _ ->
    ...
    if Int.Set.subset frels relset then
      constrain env sigma n ([], build_lambda sigma relargs ctx t) subst
    else
      raise PatternMatchingFailure

  | PMeta (Some n), _ -> merge_binding env sigma allow_bound_rels ctx n t subst
  ...
  | PApp (PMeta meta, args1), App (c2, args2) ->
    ...
    if diff >= 0 then
      ...
      | None -> subst
      | Some n -> merge_binding env sigma allow_bound_rels ctx n c subst
      in
